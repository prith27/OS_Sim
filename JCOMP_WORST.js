            ...
            if (parseInt(requestSize) < 0) {
                ...
                for (i = 0; i < numblocks; i++) {
                    if (parseInt(blocks[i]) >= parseInt(requestSize) && filled[i] == 0) {
                        if (worstIdx == -1)
                            worstIdx = i;
                        else if (parseInt(blocks[worstIdx]) < parseInt(blocks[i]))
                            worstIdx = i;
                    }
                }
                if (worstIdx != -1 && filled[worstIdx] == 0) {
                    filled[worstIdx] = 1;
                    render('Allocated Block ' + worstIdx + ' to the Request with Size ' + requestSize, document.querySelector('#requestMsg'), 1);
                    render('Fragmented Space in the Block is ' + (parseInt(blocks[worstIdx]) - requestSize), document.querySelector('#Frag'), 2);
                    alloc = worstIdx;
                }
            }
            if (alloc == -1) {
